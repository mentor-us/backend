package vn.edu.hcmus.mentor.repository.custom.impl;

import vn.edu.hcmus.mentor.domain.Task;
import vn.edu.hcmus.mentor.repository.custom.TaskRepositoryCustom;
...
import static vn.edu.hcmus.mentor.domain.QAssignee.assignee;
import static vn.edu.hcmus.mentor.domain.QChannel.channel;
import static vn.edu.hcmus.mentor.domain.QTask.task;
import static vn.edu.hcmus.mentor.domain.QUser.user;
...
public class TaskRepositoryImpl extends QuerydslRepositorySupport implements TaskRepositoryCustom {
    ...
    private <T> JPAQuery<T> findAllByChannelIdInAndAssignerIdQuery(Expression<T> expression, List<String> channelIds, String assignedId) {
        return new JPAQuery<>(em)
                .select(expression)
                .from(task)
                .join(task.group, channel).fetchJoin()
                .join(task.assignees, assignee).fetchJoin()
                .join(task.assigner, user).fetchJoin()
                .where(channel.id.in(channelIds).and(user.id.eq(assignedId)));
    }

    public BooleanExpression buildTaskFilter(List<String> channelIds, String userId, Boolean isAssignee) {
        return channel.id.in(channelIds)
                .and(user.id.eq(userId)
                        .or(task.assigner.id.eq(userId)));
    }
}