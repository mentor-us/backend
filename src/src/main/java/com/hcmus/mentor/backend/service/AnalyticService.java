...
public class AnalyticService {
    ...
    public AnalyticService(GroupRepository groupRepository, TaskRepository taskRepository, MeetingRepository meetingRepository, MessageRepository messageRepository, UserRepository userRepository, PermissionService permissionService, GroupCategoryRepository groupCategoryRepository, MongoTemplate mongoTemplate, SpringTemplateEngine templateEngine) {
    ...
    SystemAnalyticResponse getGeneralInformationForSuperAdmin() {
    ...
    SystemAnalyticResponse getGeneralInformationForAdmin(String adminId) {
        ...
        long totalTasks = tasks.stream()
                .map(task -> task.getAssigneeIds().size())
    ...
    public APIResponse<SystemAnalyticResponse> getGeneralInformationByGroupCategory(String emailUser, String groupCategoryId) {
        if (!permissionService.isAdmin(emailUser)) {
            return new APIResponse(null, INVALID_PERMISSION, "Invalid permission");
        }
        List<Group> groups;
        ...
        if (permissionService.isSuperAdmin(emailUser)) {
            ...
            activeGroups = groupRepository
                    .countByGroupCategoryAndStatus(groupCategoryId, Group.Status.ACTIVE);
        } else {
            Optional<User> userOptional = userRepository.findByEmail(emailUser);
            String adminId = null;
            if (userOptional != null) {
            ...
            activeGroups = groupRepository
                    .countByGroupCategoryAndStatusAndCreatorId(groupCategoryId, Group.Status.ACTIVE, adminId);
        }
    ...
    public APIResponse<SystemAnalyticChartResponse> getDataForChart(String emailUser,
                                                                    int monthStart,
                                                                    int yearStart,
                                                                    int monthEnd,
                                                                    int yearEnd) throws ParseException {
        if (!permissionService.isAdmin(emailUser)) {
            return new APIResponse(null, INVALID_PERMISSION, "Invalid permission");
        }
        ...
        SystemAnalyticChartResponse response = SystemAnalyticChartResponse
                .builder()
                .data(data)
                .build();
        return new APIResponse(response, SUCCESS, "");
    ...
    private List<SystemAnalyticChartResponse.MonthSystemAnalytic> getChartByMonthForSuperAdmin(LocalDate timeStart, LocalDate timeEnd) {
        ...
        for (LocalDate localDate = timeStart; localDate.isBefore(timeEnd); localDate = localDate.plusMonths(1)) {
            ...
            SystemAnalyticChartResponse.MonthSystemAnalytic monthSystemAnalytic = SystemAnalyticChartResponse.MonthSystemAnalytic
                    .builder()
    ...
    private List<SystemAnalyticChartResponse.MonthSystemAnalytic> getChartByMonthForAdmin(LocalDate timeStart, LocalDate timeEnd, String adminId) {
        List<Group> groups = groupRepository.findAllByCreatorId(adminId);
        List<String> groupIds = groups.stream().map(Group::getId)
                .collect(Collectors.toList());
        List<SystemAnalyticChartResponse.MonthSystemAnalytic> data = new ArrayList<>();
        for (LocalDate localDate = timeStart; localDate.isBefore(timeEnd); localDate = localDate.plusMonths(1)) {
            Date date = Date.from(localDate.atStartOfDay(ZoneId.systemDefault()).toInstant());
            ...
            long newMessages = messageRepository.countByGroupIdInAndCreatedDateBetween(groupIds, date, dateAfterOneMonth);
            List<Task> tasks = taskRepository.findByGroupIdInAndCreatedDateBetween(groupIds, date, dateAfterOneMonth);
            long newTasks = 0;
            for (Task task : tasks) {
                newTasks += task.getAssigneeIds().size();
            }
            long newMeetings = meetingRepository.countByGroupIdInAndCreatedDateBetween(groupIds, date, dateAfterOneMonth);
            long newUsers = userRepository.countByCreatedDateBetween(date, dateAfterOneMonth);
            SystemAnalyticChartResponse.MonthSystemAnalytic monthSystemAnalytic = SystemAnalyticChartResponse.MonthSystemAnalytic
                    .builder()
                    .month(date.getMonth() + 1)
    ...
    public APIResponse<SystemAnalyticChartResponse> getDataForChartByGroupCategory(String emailUser,
                                                                                   int monthStart,
                                                                                   int yearStart,
                                                                                   int monthEnd,
                                                                                   int yearEnd,
        ...
        for (LocalDate localDate = timeStart; localDate.isBefore(timeEnd); localDate = localDate.plusMonths(1)) {
            ...
            long newMeetings = meetingRepository.countByGroupIdInAndCreatedDateBetween(groupIds, date, dateAfterOneMonth);
            long newUsers = userRepository.countByIdInAndCreatedDateBetween(new ArrayList<>(userIds), date, dateAfterOneMonth);
            SystemAnalyticChartResponse.MonthSystemAnalytic monthSystemAnalytic = SystemAnalyticChartResponse.MonthSystemAnalytic
                    .builder()
                    .month(date.getMonth() + 1)
                    .year(date.getYear() + 1900)
    ...
    public APIResponse<GroupAnalyticResponse> getGroupAnalytic(String emailUser,
                                                               String groupId) {
        if (!permissionService.isAdmin(emailUser)) {
            return new APIResponse(null, INVALID_PERMISSION, "Invalid permission");
        }
    ...
    public ResponseEntity<Resource> generateExportGroupTable(List<GroupAnalyticResponse.Member> members, List<String> remainColumns) throws IOException {
        ...
        List<String> headers = Arrays.asList("STT", "Email", "Họ tên", "Vai trò", "Số tin nhắn", "Số công việc", "Số cuộc hẹn tham gia", "Lần hoạt động gần nhất");
    ...
    public ResponseEntity<Resource> generateExportGroupTableBySearchConditions(String emailUser, String groupId, FindUserAnalyticRequest request, List<String> remainColumns) throws IOException {
        List<GroupAnalyticResponse.Member> members = getUsersAnalyticBySearchConditions(emailUser, groupId, request);
        ResponseEntity<Resource> response = generateExportGroupTable(members, remainColumns);
        return response;
    }

    private void addMembersToAnalytics(List<GroupAnalyticResponse.Member> members, List<String> memberIds,
                                       String role, String groupId) {
        for (String memberId : memberIds) {
            ...
            long totalDoneTasks = taskRepository.countByGroupIdAndAssigneeIdsUserIdInAndAssigneeIdsStatusIn(groupId, memberId, Task.Status.DONE);
            ...
            Date lastTimeMessageMember = Optional.ofNullable(messageRepository.findFirstByGroupIdAndSenderIdOrderByCreatedDateDesc(groupId, memberId))
                    ...
                    .orElse(null);
            Date lastTimeMeetingMember = Optional.ofNullable(meetingRepository.findFirstByGroupIdAndOrganizerIdOrderByCreatedDateDesc(groupId, memberId))
    ...
    private Date getLastTimeActive(String groupId) {
        Date lastTimeMessage = Optional.ofNullable(messageRepository.findFirstByGroupIdOrderByCreatedDateDesc(groupId))
                .map(Message::getCreatedDate)
                .orElse(null);

        Date lastTimeTask = Optional.ofNullable(taskRepository.findFirstByGroupIdOrderByCreatedDateDesc(groupId))
                .map(Task::getCreatedDate)
                .orElse(null);

        Date lastTimeMeeting = Optional.ofNullable(meetingRepository.findFirstByGroupIdOrderByCreatedDateDesc(groupId))
                .map(Meeting::getCreatedDate)
                .orElse(null);
    ...
    private List<GroupGeneralResponse> getGroupGeneralAnalyticFromGroups(List<Group> groups) {
        return groups.stream()
                .map(this::getGroupGeneralAnalytic)
                .collect(Collectors.toList());
    }
    ...
    private GroupGeneralResponse getGroupGeneralAnalytic(Group group) {
        ...
        String groupId = group.getId();
        String categoryName = groupCategoryRepository.findById(group.getGroupCategory())
                .map(GroupCategory::getName).orElse(null);
        ...
        long totalDoneTasks = taskRepository.countByGroupIdAndAssigneeIdsStatusIn(groupId, Task.Status.DONE);
    ...
    public APIResponse<Page<GroupGeneralResponse>> getGroupGeneralAnalytic(String emailUser, Pageable pageRequest) {
        if (!permissionService.isAdmin(emailUser)) {
            return new APIResponse(null, INVALID_PERMISSION, "Invalid permission");
        }
        Page<Group> groups;
        ...
        Page<GroupGeneralResponse> responsesPage = new PageImpl<>(responses, pageRequest, groups.getTotalElements());
    ...
    public ResponseEntity<Resource> generateExportGroupsTable(String emailUser, List<String> remainColumns) throws IOException {
    ...
    public ResponseEntity<Resource> generateExportGroupsTableBySearchConditions(String emailUser, FindGroupGeneralAnalyticRequest request, List<String> remainColumns) throws IOException {
    ...
    public APIResponse<Page<GroupGeneralResponse>> findGroupGeneralAnalytic(String emailUser,
        ...
        Page<GroupGeneralResponse> responsesPage = new PageImpl<>(pagedResponses, pageRequest, responses.size());
        return new APIResponse(pagingResponse(responsesPage), SUCCESS, "");
    }
    ...
    private List<GroupAnalyticResponse.Member> getUsersAnalyticBySearchConditions(String emailUser,
        ...
        if (request.getTimeStart() != null && request.getTimeEnd() != null) {
            members = members.stream().filter(response ->
                            response.getLastTimeActive() != null
                                    && response.getLastTimeActive().after(request.getTimeStart())
                                    && response.getLastTimeActive().before(request.getTimeEnd()))
                    .collect(Collectors.toList());
    ...
    public APIResponse<List<GroupAnalyticResponse.Member>> findUserAnalytic(String emailUser,
                                                                            String groupId,
                                                                            FindUserAnalyticRequest request) {
        if (!permissionService.isAdmin(emailUser)) {
            return new APIResponse(null, INVALID_PERMISSION, "Invalid permission");
    ...
    public APIResponse<Map<String, String>> importData(String emailUser, MultipartFile file, String type) throws IOException {
        if (!permissionService.isAdmin(emailUser)) {
            return new APIResponse(null, INVALID_PERMISSION, "Invalid permission");
        }
    ...
    private APIResponse<List<ImportGeneralInformationResponse>> validateMultipleData(Map<String, List<String>> data) {
    ...
    public APIResponse<List<ImportGeneralInformationResponse>> importMultipleData(String emailUser, MultipartFile file) throws IOException {
        if (!permissionService.isAdmin(emailUser)) {
            return new APIResponse(null, INVALID_PERMISSION, "Invalid permission");
        }
        Map<String, List<String>> data = parseExcelFourColumns(file);
        APIResponse<List<ImportGeneralInformationResponse>> isValidMultipleData = validateMultipleData(data);
    ...
    private Map<String, List<String>> parseExcelFourColumns(MultipartFile file) throws IOException {
        ...
        for (Row row : sheet) {
            ...
            if (keyCell != null && trainingPointCell != null && hasEnglishCertCell != null && studyingPointCell != null) {
                String key = keyCell.getStringCellValue();
                DataFormatter formatter = new DataFormatter(); //creating formatter using the default locale
    ...
    public APIResponse<User> updateStudentInformation(String emailUser, String userId, UpdateStudentInformationRequest request) {
        if (!permissionService.isAdmin(emailUser)) {
            return new APIResponse(null, INVALID_PERMISSION, "Invalid permission");
        }
