...
public class VoteService {
    ...
    public VoteService(VoteRepository voteRepository,
                       ...
                       @Lazy MessageService messageService, SocketIOServer socketServer) {
    ...
    public List<VoteDetailResponse> getGroupVotes(String userId, String groupId) {
        ...
        return voteRepository.findByGroupIdOrderByCreatedDateDesc(groupId)
                .stream().map(this::fulfillChoices)
    ...
    public Vote createNewVote(String userId, CreateVoteRequest request) {
        ...
        MessageDetailResponse response = MessageDetailResponse.from(
                MessageResponse.from(message, ProfileResponse.from(sender)),
                newVote);
        socketServer.getRoomOperations(request.getGroupId())
    ...
    public VoteDetailResponse.ChoiceDetail getChoiceDetail(UserPrincipal user, String voteId, String choiceId) {
        Optional<Vote> voteWrapper = voteRepository.findById(voteId);
        if (!voteWrapper.isPresent()) {
            return null;
        }
