...
public class  GroupCategoryController {
    ...
    public APIResponse<GroupCategory> findById(@PathVariable String id) {
        ...
        return new APIResponse(groupCategoryReturn.getData(), groupCategoryReturn.getReturnCode(), groupCategoryReturn.getMessage());
    }
    ...
    @ApiResponses({
            ...
            @ApiResponse(responseCode = GroupCategoryReturnCode.NOT_ENOUGH_FIELDS_STRING, description = "Not enough required fields"),
            @ApiResponse(responseCode = GroupCategoryReturnCode.DUPLICATE_GROUP_CATEGORY_STRING, description = "Duplicate group category"),
    })
    @PostMapping
    public APIResponse<GroupCategory> create(@Parameter(hidden = true) @CurrentUser UserPrincipal userPrincipal,
    ...
    @Operation(summary = "Update an existing group category", description = "", tags = "Group Category APIs")
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "Retrieve successfully",
                    content = @Content(array = @ArraySchema(schema = @Schema(implementation = APIResponse.class)))
            ),
            @ApiResponse(responseCode = "401", description = "Need authentication"),
            @ApiResponse(responseCode = GroupCategoryReturnCode.NOT_FOUND_STRING, description = "Not found group category"),
            @ApiResponse(responseCode = GroupCategoryReturnCode.DUPLICATE_GROUP_CATEGORY_STRING, description = "Duplicate group category"),
    })
    @PatchMapping("/{id}")
    public APIResponse<GroupCategory> update(@Parameter(hidden = true) @CurrentUser UserPrincipal userPrincipal,
                              @PathVariable String id, @RequestBody UpdateGroupCategoryRequest request) {
        String email = userPrincipal.getEmail();
        GroupCategoryReturn groupCategoryReturn = groupCategoryService.update(email, id, request);
        return new APIResponse(groupCategoryReturn.getData(), groupCategoryReturn.getReturnCode(), groupCategoryReturn.getMessage());
    }

    @Operation(summary = "Delete an existing group category", description = "", tags = "Group Category APIs")
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "Retrieve successfully",
                    content = @Content(array = @ArraySchema(schema = @Schema(implementation = APIResponse.class)))
            ),
    ...
    public APIResponse delete(@Parameter(hidden = true) @CurrentUser UserPrincipal userPrincipal,
        ...
        GroupCategoryReturn groupCategoryReturn = groupCategoryService.delete(email, id, newGroupCategoryId);
        return new APIResponse(groupCategoryReturn.getData(), groupCategoryReturn.getReturnCode(), groupCategoryReturn.getMessage());
    }
    @Operation(summary = "Find group categories", description = "Find group categories with multiple filters", tags = "Group Category APIs")
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "Retrieve successfully",
                    content = @Content(array = @ArraySchema(schema = @Schema(implementation = APIResponse.class)))
            )
    ...
    public APIResponse<Page<Group>> get(@Parameter(hidden = true) @CurrentUser UserPrincipal userPrincipal,
        ...
        GroupCategoryReturn groupCategoryReturn = groupCategoryService.findGroupCategories(email, request, page, size);
        return new APIResponse(pagingResponse((Page<GroupCategory>) groupCategoryReturn.getData()), groupCategoryReturn.getReturnCode(), groupCategoryReturn.getMessage());
    }
    @Operation(summary = "Delete multiple existing group categories", description = "", tags = "Group Category APIs")
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "Retrieve successfully",
                    content = @Content(array = @ArraySchema(schema = @Schema(implementation = APIResponse.class)))
            ),
    ...
    public APIResponse deleteMultiple(@Parameter(hidden = true) @CurrentUser UserPrincipal userPrincipal,
        ...
        String newGroupCategoryId = request.getNewGroupCategoryId();
        GroupCategoryReturn groupCategoryReturn = groupCategoryService.deleteMultiple(email, ids, newGroupCategoryId);
        return new APIResponse(groupCategoryReturn.getData(), groupCategoryReturn.getReturnCode(), groupCategoryReturn.getMessage());
    }

    @Operation(summary = "Export group categories table by search conditions",
            description = "Export group categories table", tags = "Group Category APIs")
    @ApiResponses({
    ...
    public ResponseEntity<Resource> exportBySearchConditions(@Parameter(hidden = true) @CurrentUser UserPrincipal userPrincipal,
        ...
        FindGroupCategoryRequest request = new FindGroupCategoryRequest(name, description, status);
        ResponseEntity<Resource> response = groupCategoryService.generateExportTableBySearchConditions(userPrincipal.getEmail(), request, remainColumns);
        return response;
    }

    @Operation(summary = "Export group categories table",
            description = "Export group categories table", tags = "Group Category APIs")
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "Update successfully",
                    content = @Content(array = @ArraySchema(schema = @Schema(implementation = APIResponse.class)))
    ...
    public ResponseEntity<Resource> export(@Parameter(hidden = true) @CurrentUser UserPrincipal userPrincipal,
                                           @RequestParam(defaultValue = "") List<String> remainColumns) throws IOException {
        ResponseEntity<Resource> response = groupCategoryService.generateExportTable(userPrincipal.getEmail(), remainColumns);
        return response;
    }

    @Operation(summary = "Get all permission's system",
            description = "Get all permission's system", tags = "Group Category APIs")
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "Update successfully",
                    content = @Content(array = @ArraySchema(schema = @Schema(implementation = APIResponse.class)))
            ),
