...
public class Task implements IRemindable, Serializable {
    ...
    public Date getReminderDate() {
        LocalDateTime localDateTime = LocalDateTime.ofInstant(deadline.toInstant(), ZoneId.systemDefault());
    ...
    public List<String> getAllAssigneeIds() {
        return assigneeIds.stream().map(Assignee::getUserId)
    ...
    public void update(UpdateTaskRequest request) {
        ...
        if (request.getUserIds() != null) {
            List<Task.Assignee> assignees = assigneeIds.stream()
                    .filter(assignee ->
                            request.getUserIds().contains(assignee.getUserId()))
                    .collect(Collectors.toList());
