...
public class GetNotesByUserIdQueryHandler implements Command.Handler<GetNotesByUserIdQuery, Page<NoteDto>> {
    ...
    @Override
    public Page<NoteDto> handle(GetNotesByUserIdQuery command) {
        ...
        Page<NoteDto> adbc = new PageImpl<>(notes.getContent().stream().map(note -> {
