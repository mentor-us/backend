...
public class OAuth2AuthenticationSuccessHandler extends SimpleUrlAuthenticationSuccessHandler {
    ...
    OAuth2AuthenticationSuccessHandler(TokenProvider tokenProvider, AppProperties appProperties,
                                       OAuth2AuthorizationRequestRepository OAuth2AuthorizationRequestRepository, AuthService authService, PermissionService permissionService) {
    ...
    protected String determineTargetUrl(HttpServletRequest request, HttpServletResponse response,
        ...
        if (targetUrl.contains("auth/redirect")
                && !permissionService.isAdmin(userPrincipal.getEmail())) { // Check if redirect to Admin Page
