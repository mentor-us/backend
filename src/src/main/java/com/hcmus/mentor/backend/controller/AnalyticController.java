...
public class AnalyticController {
    ...
    @Operation(summary = "Get general information of system / groupCategory",
    ...
    @GetMapping({"/", ""})
    public APIResponse<SystemAnalyticResponse> get(@ApiIgnore @CurrentUser UserPrincipal userPrincipal,
    ...
    @Operation(summary = "Get general information of system / groupCategory(by month)",
    ...
    @GetMapping({"/chart"})
    public APIResponse<SystemAnalyticChartResponse> getDataForChart(@ApiIgnore @CurrentUser UserPrincipal userPrincipal,
                                                                    @RequestParam int monthStart,
                                                                    @RequestParam int yearStart,
                                                                    @RequestParam int monthEnd,
                                                                    @RequestParam int yearEnd,
                                                                    @RequestParam(required = false) String groupCategoryId) throws ParseException {
        String emailUser = userPrincipal.getEmail();
        return groupCategoryId == null ?
                analyticService.getDataForChart(emailUser, monthStart, yearStart, monthEnd, yearEnd) :
                analyticService.getDataForChartByGroupCategory(emailUser, monthStart, yearStart, monthEnd, yearEnd, groupCategoryId);
    }

    @Operation(summary = "Get group's analytic",
    ...
    @GetMapping({"/{groupId}"})
    public APIResponse<GroupAnalyticResponse> getGroupAnalytic(@ApiIgnore @CurrentUser UserPrincipal userPrincipal,
                                                               @PathVariable String groupId) {
        String emailUser = userPrincipal.getEmail();
        return analyticService.getGroupAnalytic(emailUser, groupId);
    }
    @Operation(summary = "Export groups general analytic",
    ...
    public ResponseEntity<Resource> exportGroups(@ApiIgnore @CurrentUser UserPrincipal userPrincipal,
    ...
    @Operation(summary = "Export groups general analytic by search conditions",
            description = "Export groups general analytic",
            tags = "Analytic APIs")
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "Export successfully",
    ...
    public ResponseEntity<Resource> exportGroupsBySearchConditions(@ApiIgnore @CurrentUser UserPrincipal userPrincipal,
                                                   @RequestParam(required = false) String groupName ,
                                                   @RequestParam(required = false) Group.Status status,
                                                   @RequestParam(required = false) String groupCategory,
                                                   @RequestParam(required = false)
                                                       @DateTimeFormat(pattern = "yyyy-MM-dd'T'HH:mm:ss") Date timeStart,
                                                   @RequestParam(required = false)
                                                       @DateTimeFormat(pattern = "yyyy-MM-dd'T'HH:mm:ss") Date timeEnd,
                                                   @RequestParam(defaultValue = "") List<String> remainColumns) throws IOException {
        String emailUser = userPrincipal.getEmail();
    ...
    @Operation(summary = "Export group's analytic",
    ...
    public ResponseEntity<Resource> exportGroup(@ApiIgnore @CurrentUser UserPrincipal userPrincipal,
                                                @PathVariable String groupId,
                                                @RequestParam(defaultValue = "") List<String> remainColumns ) throws IOException {
        String emailUser = userPrincipal.getEmail();
    ...
    @Operation(summary = "Export group's analytic by search conditions",
            description = "Export group's analytic",
            tags = "Analytic APIs")
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "Export successfully",
    ...
    public ResponseEntity<Resource> exportGroupBySearchCondotions(@ApiIgnore @CurrentUser UserPrincipal userPrincipal,
                                                                  @PathVariable String groupId,
    ...
    @GetMapping({"/groups"})
    public APIResponse<Page<GroupGeneralResponse>> all(@ApiIgnore @CurrentUser UserPrincipal userPrincipal,
    ...
    @Operation(summary = "Import multiple training point",
    ...
    public APIResponse<Map<String, String>> importTrainingPoint(@ApiIgnore @CurrentUser UserPrincipal userPrincipal,
                                                                 @RequestBody MultipartFile file) throws IOException {
        String emailUser = userPrincipal.getEmail();
        return analyticService.importData(emailUser, file, "TRAINING_POINT");
    }

    @Operation(summary = "Import multiple has English certs",
    ...
    public APIResponse<Map<String, String>> importHasEnglishCert(@ApiIgnore @CurrentUser UserPrincipal userPrincipal,
                                                                @RequestBody MultipartFile file) throws IOException {
        String emailUser = userPrincipal.getEmail();
        return analyticService.importData(emailUser, file, "HAS_ENGLISH_CERT");
    }

    @Operation(summary = "Import multiple studying points",
    ...
    public APIResponse<Map<String, String>> importStudyingPoint(@ApiIgnore @CurrentUser UserPrincipal userPrincipal,
                                                                @RequestBody MultipartFile file) throws IOException {
        String emailUser = userPrincipal.getEmail();
        return analyticService.importData(emailUser, file, "STUDYING_POINT");
    }

    @Operation(summary = "Import multiple training points, has English certs studying points",
    ...
    public APIResponse<List<ImportGeneralInformationResponse>> importMultiple(@ApiIgnore @CurrentUser UserPrincipal userPrincipal,
                                                                              @RequestBody MultipartFile file) throws IOException {
        String emailUser = userPrincipal.getEmail();
        return analyticService.importMultipleData(emailUser, file);
    }

    @Operation(summary = "Update training point, has English cert, studying point",
    ...
    public APIResponse<User> update(@ApiIgnore @CurrentUser UserPrincipal userPrincipal,
                                            ...
                                            @RequestBody UpdateStudentInformationRequest request) throws IOException {
        String emailUser = userPrincipal.getEmail();
        return analyticService.updateStudentInformation(emailUser, userId, request);
    }

    @Operation(summary = "Find groups in analytic",
    ...
    @GetMapping({"/find"})
    public APIResponse<Page<GroupGeneralResponse>> findGroups(@ApiIgnore @CurrentUser UserPrincipal userPrincipal,
                                                                @RequestParam(defaultValue = "0") int page,
                                                                @RequestParam(defaultValue = "25") int pageSize,
                                                                @RequestParam(required = false) String groupName ,
                                                                @RequestParam(required = false) String groupCategory,
                                                                ...
                                                                @RequestParam(required = false)
                                                                  @DateTimeFormat(pattern = "yyyy-MM-dd'T'HH:mm:ss") Date timeStart,
                                                                @RequestParam(required = false)
                                                                  @DateTimeFormat(pattern = "yyyy-MM-dd'T'HH:mm:ss") Date timeEnd) throws IOException {
        String emailUser = userPrincipal.getEmail();
        FindGroupGeneralAnalyticRequest request = new FindGroupGeneralAnalyticRequest(groupName, groupCategory, status, timeStart, timeEnd);
        Pageable pageRequest = PageRequest.of(page, pageSize, Sort.by(Sort.Direction.DESC, "createdDate"));
    ...
    @Operation(summary = "Find users in analytic",
    ...
    public APIResponse<List<GroupAnalyticResponse.Member>> findUsers(@ApiIgnore @CurrentUser UserPrincipal userPrincipal,
                                                                     @PathVariable String groupId,
                                                                     @RequestParam(required = false) String name,
                                                                     @RequestParam(required = false) String email,
                                                                     @RequestParam(required = false) FindUserAnalyticRequest.Role role,
    ...
    @Operation(summary = "Export group report",
    ...
    @GetMapping("/report")
    public ResponseEntity<?> getGroupReport(@ApiIgnore @CurrentUser UserPrincipal userPrincipal,
                                            @RequestParam String groupId,
                                            HttpServletRequest request,
                                            HttpServletResponse response) {
        WebContext context = new WebContext(request, response, servletContext);
        String reportHtml = analyticService
                .exportGroupReport(userPrincipal.getEmail(), groupId, context);
        if (reportHtml == null) {
    ...
    @Operation(summary = "Export group log",
    ...
    @GetMapping("/log")
    public ResponseEntity<?> getGroupLog(@ApiIgnore @CurrentUser UserPrincipal userPrincipal,
                                         @RequestParam String groupId,
