...
public class ReminderService {
    ...
    public ReminderService(ReminderRepository reminderRepository, UserRepository userRepository, GroupRepository groupRepository, MailService mailService, FirebaseMessagingManager firebaseMessagingManager) {
    ...
    public void sendReminders() throws FirebaseMessagingException {
        ...
        for (Reminder reminder : reminders) {
            ...
            if(reminder.getType() == Reminder.ReminderType.MEETING){
                body = "Lịch hẹn " + reminder.getName() + " sẽ diễn ra lúc " + reminder.getProperties().get("dueDate");
            }
            else if(reminder.getType() == Reminder.ReminderType.TASK){
                body = "Công việc " + reminder.getName() + " sẽ tới hạn lúc " + reminder.getProperties().get("dueDate");
            }
