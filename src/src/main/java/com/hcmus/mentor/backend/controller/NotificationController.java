...
public class NotificationController {
    ...
    @GetMapping(value = {"", "/"})
    public APIResponse all(@CurrentUser UserPrincipal userPrincipal,
                           @RequestParam(defaultValue = "0") int page,
                           @RequestParam(defaultValue = "25") int pageSize) {
        Map<String, Object> response = notificationService
                .getOwnNotifications(userPrincipal.getId(), page, pageSize);
        return APIResponse.success(response);
    }

    @Operation(summary = "Retrieve existing notification", description = "", tags = "Notification APIs")
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "Get successfully",
                    content = @Content(array = @ArraySchema(schema = @Schema(implementation = APIResponse.class)))
            ),
    ...
    @Operation(summary = "Reply notification",
            description = "The receiver of notification have to reply to this one (If need)\n" +
    ...
    @Operation(summary = "Mobile app subscribe to Notification Server",
    ...
    @Operation(summary = "Mobile app subscribe to topic",
    ...
    public APIResponse<Void> subscribeTopic(@RequestParam String topic,
                                            @PathVariable String token) {
        return APIResponse.notFound(404);
    }

    @Operation(summary = "Get unread notifications number",
